...
# csv_titles = [
# f'H:/stOOrz-Mathematical-Modelling-Group/IMMC_2022_International/Programming/simulation_of_plane_boarding/ours/with_satisfaction/output_csv_random/log{i}.csv'
...
targx = [7, 4, 1, 11, 10, 10, 9, 6, 3, 9, 9, 7, 5, 8, 10, 1, 8, 7, 10, 8, 1, 7, 3, 5, 10, 5, 5, 2, 11, 7, 2, 10, 9, 11,
...
targy = [-1, -1, -3, 2, -1, 2, -2, 2, -3, -1, 3, 2, 2, -2, -2, -1, 3, 1, 1, 1, -2, 3, 1, -2, -3, 3, 1, 3, -1, -3, -3, 3,
...
def get_offering_seat_time(pasng):
    ans = 0
    for i in range(pasng.sequence):
        if passenger_set[i].target_x == pasng.target_x and passenger_set[i].target_y * pasng.target_y > 0 and abs(
                passenger_set[i].target_y) > abs(pasng.target_y):
            ans = max(ans, abs(passenger_set[i].target_y))
    return ans * STANDARD_SEATING_TIME


def get_wholeset_average_variation():
    ans = 0
    for i in range(PASSENGER_NUM):
        list = []
        count = 0
        sum = 0
        varsum = 0
        for j in range(PASSENGER_NUM):
            if passenger_set[i].target_x == passenger_set[j].target_x and i != j and passenger_set[i].target_y * \
                    passenger_set[i].target_y > 0:  # 在同一行
                count = count + 1
                list.append(passenger_set[j].sequence)
                sum = sum+passenger_set[j].sequence
        for k in range(count):
            varsum = varsum+(list[k]-(sum/count))**2
        ans = ans+ np.sqrt(varsum/count)
    return ans/PASSENGER_NUM

def get_variation(pasng):
    list = []
    count = 0
    sum = 0
    varsum = 0
    for j in range(PASSENGER_NUM):
        if pasng.target_x == passenger_set[j].target_x and i != j and pasng.target_y * \
                pasng.target_y > 0:  # 在同一行
            count = count + 1
            list.append(passenger_set[j].sequence)
            sum = sum + passenger_set[j].sequence
    for k in range(count):
        varsum = varsum + (list[k] - (sum / count)) ** 2
    return np.sqrt(varsum / count)

already_done_no = []  # 已经搞定的乘客的编号
time_step = 0
# while not is_everyone_seated():

queueing_time = [0 for i in range(PASSENGER_NUM)]
    ...
    for i in range(PASSENGER_NUM):
        ...
        if i not in already_done_no:  # 如果 i 还没坐下
            ...
            if passenger_set[i].state == 0:
                ...
                passenger_set[i].curx = passenger_set[i].curx + 0.08
                passenger_set[i].velocity = get_velocity(passenger_set[i])
                passenger_set[i].density = get_visibility(passenger_set[i])
                passenger_set[i].curcell = get_current_cell(passenger_set[i])
                ...
                queueing_time[i] = queueing_time[i] + SIMULATION_TAU
                # 等待哦
...
offeringseat_time = []
for i in range(PASSENGER_NUM):
    offeringseat_time.append(get_offering_seat_time(passenger_set[i]))
variations = []
for i in range(PASSENGER_NUM):
    variations.append(get_variation(passenger_set[i]))
f.write('queueing time:')
f.write(str(queueing_time))
f.write('offering seat time:')
f.write(str(offeringseat_time))
f.write('variation between rows:')
f.write(str(variations))
